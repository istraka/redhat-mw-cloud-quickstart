...
echo "$EAP_HOME/bin/standalone.sh -c standalone-full.xml -bmanagement $Public_IP &" | adddate >> jbosseap.install.log
$EAP_HOME/bin/standalone.sh -c standalone-full.xml -bmanagement $Public_IP >> jbosseap.install.log 2>&1 &
