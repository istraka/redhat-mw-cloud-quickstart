...
echo "ooooo      RED HAT JBoss EAP 7.2 RPM INSTALL      ooooo" >> jbosseap.install.log
...
echo "subscription-manager register --username $RHSM_USER" >> jbosseap.install.log
subscription-manager register --username $RHSM_USER --password $RHSM_PASSWORD  >> jbosseap.install.log 2>&1
flag=$?; if [ $flag != 0 ] ; then echo  "Registration Failed" >> jbosseap.install.log; exit $flag;  fi
echo "subscription-manager attach --pool=${RHSM_POOL}" >> jbosseap.install.log
...
then 
    ...
    echo "subscription-manager attach --pool=$7"  >> jbosseap.install.log
...
echo "$EAP_HOME/bin/add-user.sh -u $JBOSS_EAP_USER -g 'guest,mgmtgroup'" >> jbosseap.install.log
