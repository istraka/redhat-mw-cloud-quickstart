# JBoss EAP 7.3 on RHEL 8.0 (clustered, multi-VM)
...
This Azure Resource Manager (ARM) template creates all the Azure compute resources to run JBoss EAP 7.3 cluster running 'n' number of RHEL 8.0 VMs where 'n' is decided by the user and all the VMs are added to the backend pool of a Load Balancer. The following resources are created by this template:
...
1. Login to the provisioned virtual machine through SSH. You can retrieve the Public IP of the VM using the Azure portal VM overview page. Under settings section go to Networking and copy the NIC Public IP.
